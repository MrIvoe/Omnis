...
| 4 | Database | 🟡 In progress — still JSON-file storage, not an indexed DB. §41's "atomic writes" requirement is now real across every store that persists user-authored/derived data: `LibraryStore` (atomic + debounced, prior work), `RecoveryJournal` (atomic, this session), and now `PlaylistStore`/`PlayHistoryStore` (atomic, this session) — all write to a sibling `.tmp` file and rename over the real path, so a crash mid-write can no longer corrupt/truncate any of them. `RemoteTextStore` (theme/layout installs) deliberately left as-is — each install is one independent file, not shared mutable state, so the blast radius of an interrupted write is already contained. Still no multi-source libraries, no SQL-like query layer, no integrity check/backup rotation/migration system. |
...
| 8 | Error handling | 🟡 In progress — decoder-error auto-skip, plugin-call sandboxing, plugin-installer download/zip-bomb size limits, and (new) a per-file scan failure no longer aborting the whole library scan are all in place — each one a real instance of §60's "one bad entry breaks everything" anti-pattern found and closed. The spec's full failure-mode checklist hasn't been run exhaustively against every subsystem — this is audit-as-you-go, not a completed sweep. |
