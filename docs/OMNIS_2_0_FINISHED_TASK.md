...
| 15 | Ratings | ⬜ **Genuine 0% gap, confirmed** — zero references to "rating" anywhere in this repo (`lib/`). §9 calls for a 0–5 rating alongside favorites; no plugin or field for it exists. The natural shape (a `RatingsPlugin` alongside the existing `FavoritesPlugin`) lives in the separate `Omnis-Plugins` repo, which is why this wasn't picked up as this session's first Phase 2 item — cross-repo plugin work (a new plugin there, a version bump + `bundled_plugins.dart` registration here) is a distinct, larger unit of work than an in-repo fix. |
