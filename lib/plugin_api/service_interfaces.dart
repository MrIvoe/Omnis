...
/// concrete plugin type. `lib/core/` never names a concrete plugin, but
/// it never named these interfaces either — `ServiceRegistry` and
/// `EventBus` (both in `lib/core/`) are the generic *mechanism* for
/// registering/calling a capability, and that mechanism is genuinely
/// stable: it hasn't changed once across six interfaces added so far, and
/// has no reason to change for a seventh. The interfaces themselves
/// (`ILyricsProvider`, `IQueueBuilder`, ...) are *not* that — they're
/// capability-specific knowledge that keeps growing as the plugin
/// ecosystem grows, and putting them in `lib/core/` would mean the kernel
/// grows forever too, one file per feature, which is exactly what a
/// "small, stable core" is supposed to avoid. `lib/plugin_api/` is the
/// layer that's *allowed* to grow: it depends on `lib/core/` (for
/// `BaseTrack` and nothing else) but `lib/core/` never depends on it back.
/// A plugin implements an interface from here; a caller — UI code,
/// another plugin — imports it from here too, and neither one needs to
/// import the other's concrete file, or touch `lib/core/` at all, to add
