...
/// heart, Musicolet's "top rated", Poweramp's star). Deliberately just a
/// thin API surface over `AppSettings.favoriteTrackIds`, the same split
/// `TagEditorPlugin` uses for `autoTaggedTrackIds` -- the plugin owns the
/// behavior, `AppSettings` owns persistence, and nothing about "is this
/// track favorited" requires the plugin to be enabled to be readable by
/// something else that needs to know (Library sort/filter, a future
/// Favorites playlist).
class FavoritesPlugin extends MusicPlugin {
  bool isFavorite(String trackId) =>
      AppSettings.instance.favoriteTrackIds.contains(trackId);

  Future<void> setFavorite(String trackId, bool favorite) async {
    final ids = AppSettings.instance.favoriteTrackIds;
    final changed = favorite ? ids.add(trackId) : ids.remove(trackId);
    if (!changed) return;
    await AppSettings.instance.setFavoriteTrackIds(ids);
  ...
  int get count => AppSettings.instance.favoriteTrackIds.length;
  ...
  Future<void> clearAll() async {
    ...
    await AppSettings.instance.setFavoriteTrackIds({});
    for (final id in ids) {
