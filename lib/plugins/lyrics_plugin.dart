...
/// track ID.
///
/// Plain lyrics persist via [AppSettings] and can be entered through
/// [LyricEditDialog] — previously nothing in the UI ever called
/// [setLyric], so this plugin's actual purpose (storing and showing
/// lyrics) was unreachable by a user no matter how the karaoke/lyrics
/// settings were configured. Timed lyrics ([setTimedLyric]) still have no
/// entry UI — writing a synced-lyric editor (per-line timestamps) is a
/// separate, larger feature — but [currentLyricFor] already uses them
/// correctly whenever a caller sets them programmatically.
///
...
class LyricsPlugin extends MusicPlugin implements ILyricsProvider {
  ...
  void setLyric(String trackId, String lyric) {
    ...
    AppSettings.instance.setStoredLyrics(_lyrics);
  }
  ...
  Future<void> initialize() async {
    _lyrics
      ..clear()
      ..addAll(AppSettings.instance.storedLyrics);
