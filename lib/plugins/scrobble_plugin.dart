...
/// Records real play history, persisted via `AppSettings`. Previously this
/// only ever lived in memory, capped at the last 10 plays as pre-joined
/// display strings — so every "recently played"/"most played" idea (which
/// every named competitor has under some name) had no real data to work
/// from, and a restart erased everything anyway.
///
...
class ScrobblePlugin extends MusicPlugin implements IPlayHistoryProvider {
  ...
  Future<void> initialize() async {
    _history =
        AppSettings.instance.playHistory.map(PlayRecord.fromJson).toList();
  ...
  Future<void> onTrackStart(BaseTrack track) async {
    ...
    await AppSettings.instance
        .setPlayHistory(_history.map((r) => r.toJson()).toList());
  }
  ...
  Future<void> clearHistory() async {
    _history = [];
    await AppSettings.instance.setPlayHistory(const []);
  }
