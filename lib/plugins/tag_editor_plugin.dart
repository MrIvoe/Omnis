...
class TagEditorPlugin extends MusicPlugin implements IFileTagWriter {
  ...
  // skipped unless the user explicitly asks to redo it.

  bool wasAutoTagged(String trackId) =>
      AppSettings.instance.autoTaggedTrackIds.contains(trackId);

  Future<void> markAutoTagged(String trackId) async {
    final ids = AppSettings.instance.autoTaggedTrackIds;
    if (ids.add(trackId)) {
      await AppSettings.instance.setAutoTaggedTrackIds(ids);
    }
  }
  ...
  Future<void> clearAllAutoTagged() =>
      AppSettings.instance.setAutoTaggedTrackIds({});
  ...
  List<String> splitArtists(String raw) {
    ...
    for (final sep in separators) {
      ...
      working = working.replaceAll(
        RegExp(RegExp.escape(sep), caseSensitive: false),
        ' ',
      );
    }
    return working
...
/// Unused import guard: keeps `dart:convert` available for callers that
/// need to jsonEncode a TrackTags snapshot (e.g. a future export feature)
/// without every editor complaining about an unused import in the
/// meantime. Intentionally trivial.
// ignore: unused_element
String _unused() => jsonEncode(const {});
